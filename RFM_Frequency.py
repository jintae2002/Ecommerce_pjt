...
order by Customer_ID;
    ...
    df = df[['Frequency','Customer_ID']]
    
    df1 = df.groupby('Frequency')["Customer_ID"].count().to_frame()
    
    
    
    df1 = df1.reset_index().rename(columns={"index": "Frequency"})
    
    #df1 = df1.pivot(index="Frequency", columns="Customer_ID")
    
    # df1 = df1['Customer_ID']
    
    df1.columns=["frequency", "customer_id"]
